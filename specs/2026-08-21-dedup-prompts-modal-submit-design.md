...
- **自有/引用判定**：采集时对每个配置帖调一次 fxtwitter API 取其自有 media ID 列表；
  yt-dlp 提取出的条目中，命中自有列表的归本组，未被任何配置帖认领的引用视频附加到当前组，
  已被其他配置帖认领的丢弃（如 Edimakor 帖里的夏一跳视频归夏一跳组、Unique Wallpaper 两帖互引各自归位）
- **全局去重**：以 media ID 为全局主键兜底去重，保留先出现条目
- fxtwitter 失败时降级为旧逻辑（全部归当前组），打印警告
...
## 修订 3（2026-08-21，最终）：彻底移除 AI/提示词

1. **采集范围**：仅配置帖子本身的内容与视频，不做时间线爬取（实测 yt-dlp 不支持 x.com 时间线 URL）。来源 = 配置帖子 + 用户 GitHub Issue 上报。
2. **提示词功能整体移除**：数据不再含 `prompt`/`prompt_source` 字段；弹窗不展示提示词；搜索不含提示词。删除 `scripts/enrich_prompts.py` 与 `tests/test_enrich.py`。
3. **主视频规则**：组内第 2 个视频为默认展示项；仅 1 个视频时取第 1 个。
4. **筛选栏**：搜索默认收起为 🔍 图标按钮，点击展开输入框（Esc 收起）。
